...
class ConceptDriftDetectionMethod:
    ...
    def __init__(self, method):
        self.monitored_variable_file = open("results/" + method + '_F1Scores.txt', "w")
        self.drifts_file = open("results/" + method + '_Drifts.txt', "w")
    ...
    def makePlot(self):
        with open("results/" + self.method_name + '_F1Scores.txt', "r") as file:
            string = file.read()
        errors = np.fromstring(string, dtype=float, sep=" ")
        with open("results/" + self.method_name + '_Drifts.txt', "r") as file:
            string = file.read()
